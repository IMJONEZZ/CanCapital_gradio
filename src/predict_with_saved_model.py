...
def engineer_prediction_features(df):
    ...
    try:
        ...
        date_columns = ['funded_date', 'pg_date_of_birth', 'last_payment_date', 
                       'demand_letter_sent_date', 'collection_case_open_date', 'collection_case_close_date']
        ...
        if "pg_date_of_birth" in processed_df.columns and "funded_date" in processed_df.columns:
            processed_df["borrower_age"] = (
                processed_df["funded_date"] - pd.to_datetime(processed_df["pg_date_of_birth"], errors="coerce")
            ).dt.days / 365.25
        else:
            processed_df["borrower_age"] = np.nan
        
        # time between funding and case open
        if "funded_date" in processed_df.columns and "collection_case_open_date" in processed_df.columns:
        ...
        if "last_payment_date" in processed_df.columns and "collection_case_close_date" in processed_df.columns:
            processed_df["days_lastpay_to_close"] = (
                processed_df["collection_case_close_date"] - processed_df["last_payment_date"]
            ).dt.days
        else:
        ...
        for col in ["has_collection_case_close_date", "has_demand_letter_sent_date"]:
            if col in processed_df.columns:
                print(f"  {col}: {processed_df[col].sum()}/{len(processed_df)} have the date")
    ...
    try:
        ...
        for col in numeric_columns:
            if processed_df[col].isnull().sum() > 0:
                ...
                print(f"Filled {processed_df[col].isnull().sum()} missing values in '{col}' with mean: {mean_value:.2f}")
        ...
        if hasattr(model, 'predict'):
            ...
            try:
                if hasattr(model, 'feature_names_in_'):
                    ...
                    print(f"Model expects features ({len(expected_features)}): {list(expected_features)}")
                    ...
                    if missing_cols:
                        ...
                        for col in missing_cols:
                            if 'age' in col.lower():
                                data[col] = 35.0
                                print(f"Added missing column '{col}' with default value 35.0")
                            elif 'days' in col.lower():
                                data[col] = 30.0
                                print(f"Added missing column '{col}' with default value 30.0")
                            elif 'has_' in col:
                                data[col] = 1
                                print(f"Added missing column '{col}' with default value 1")
                            else:
                                data[col] = 0.5
                                print(f"Added missing column '{col}' with default value 0.5")
            ...
            if hasattr(model, 'predict_proba'):
                ...
                try:
                    ...
                    if proba.shape[1] == 2:
                        ...
                        for i, (pred, prob_neg, prob_pos) in enumerate(zip(predictions, proba[:, 0], proba[:, 1])):
                            confidence = max(prob_neg, prob_pos)
                            print(f"Sample {i+1}: Prediction = {pred} (Confidence: {confidence:.3f})")
                    else:
                        print(f"Multi-class Classification Results:")
                        classes = model.classes_
                        for i, pred in enumerate(predictions):
                            class_idx = list(classes).index(pred) if pred in classes else 0
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python predict_with_saved_model.py <model_directory> <data_file.csv>")
        print("Example: python predict_with_saved_model.py ./trained_models/model_20251113_163053 new_data.csv")
        sys.exit(1)
    ...
    model_priority = ['best_model.pkl', 'gbr_model.pkl', 'linearreg_model.pkl', 'rf_model.pkl', 'model.pkl']
        ...
        if any(priority in model_filename for priority in model_priority):
            try:
                ...
                if model is not None:
                    predictions, probabilities = make_predictions(model, processed_data, display_name)
                        ...
                        with open(output_file, 'w') as f:
                            ...
                            if probabilities is not None:
                                ...
                                if probabilities.shape[1] == 2:
                                    ...
                                    for i, (pred, prob_neg, prob_pos) in enumerate(zip(predictions, probabilities[:, 0], probabilities[:, 1])):
                                        confidence = max(prob_neg, prob_pos)
                                        f.write(f"Sample {i+1}: Prediction = {pred} (Confidence: {confidence:.3f})\n")
                                else:
                                    # Multi-class
                                    classes = model.classes_
                                    for i, pred in enumerate(predictions):
                            ...
                            if probabilities is None:
                                ...
                                f.write(f"Statistics: Mean={stats['mean']:.2f}, Std={stats['std']:.2f}, Min={stats['min']:.2f}, Max={stats['max']:.2f}\n")
